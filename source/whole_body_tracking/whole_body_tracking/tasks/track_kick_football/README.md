...
右脚推射参考动作（`motions/kick_football/right_kick.npz`），策略需要根据**带噪声的
...
| 变量 | 作用 | 阶段一 | 阶段二 |
|---|---|---|---|
| `KICK_MOTION_WEIGHT` | 6 项跟踪奖励全局缩放 | 1.0 | **0.2 → 0**（逐步压低） |
| `KICK_GOAL_WEIGHT` | 4 项球/目标奖励全局缩放 | 0 | 1.0 |
...
└── mdp/
    ...
    ├── observations.py # ball_state_b（actor）/ ball_velocity_b（critic）
    ├── rewards.py      # GOAL 组：progress / contact / speed
